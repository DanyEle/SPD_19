...
int main () {
	...
	tbb::task_arena limited(AMOUNT_THREADS);
  	...
  	concurrent_hash_map<int, int> histogram_row;


	limited.execute([b, &a, histogram_row]{
	tbb::task_group tg;


	 tg.run([b, &a, histogram_row]{ // run in task group
			  //loop over the matrix and compute the amount of iterations it takes
			  	//for the mandelbrot function to converge
			  	tbb::parallel_for(0, AMOUNT_ROWS, [b, &a, histogram_row](int row){

			  	//Key: column index
			  	//Value: amount of iterations for that column

			  				tbb::parallel_for(0, AMOUNT_COLUMNS, [b, &a, row, histogram_row](int col){
			  					...
			  					//Iters is the value, col is the row
			  					histogram_row.insert(iters, col);
	...
	//print out the values stored in the hash map
